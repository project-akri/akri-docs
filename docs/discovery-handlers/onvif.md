...
1. The sample broker first looks for files under `CREDENTIAL_DIRECTORY` with file name "username*<device_uuid>" for username and "password*<device*uuid>" for password, where device_uuid is the device uuid string that replaces all '-' with underscore '*'.
   If file "username*<device_uuid>" exists, but "password*<device_uuid>" doesn't exist, the sample broker uses empty string as password. The sample broker also looks for
